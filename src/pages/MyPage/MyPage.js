...
function MyPage() {
  ...
  useEffect(() => {
    fetch(
      'http://ec2-43-201-82-36.ap-northeast-2.compute.amazonaws.com:8080/questions',
    )
      .then(res => {
        return res.json();
      })
  ...
  useEffect(() => {
    if (user && user.memberid && user.authorization) {
      axios
        ...
        <QuestionContainer>
          ...
          <QuestionInfo>
            {questionsData
              ...
              .map(({ questionId, title, createdAt }, index) => {
                const postNumber = questionOffset + index + 1;
                return (
                  <li key={questionId}>
                    <div className="qcontainer">
                      <span className="qnumber">
                        {postNumber.toString().padStart(2, '0')}
                      </span>
                      <Link
                        className="qlist"
        ...
        <AnswerContainer>
          ...
          <AnswerInfo>
            {answersData
              ...
              .map(({ questionId, body, createdAt }, index) => {
                const postNumber = answerOffset + index + 1;
                return (
                  <li key={questionId}>
                    <div className="acontainer">
                      <span className="anumber">
                        {postNumber.toString().padStart(2, '0')}
                      </span>
                      <Link
                        className="alist"
